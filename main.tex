...
\title{Developing a standard for data sharing solutions}
\author{Maurice Verheesen}
\date{September 2019 \\ v0.87 DRAFT}
...
% notes 15-09-2019:
% verdedigen 'trias politica'. Grijpt terug op modulaire bouw en op scheiding der machten. Uitleggen wat de macht is
% van een identificatie partij, storage partij en authorisatie partij. Uitleggen dat de scheiding zorgt dat men
% meerdere afspraken stelsel met elkaar kan verbinden, mits de koppel vlakken bekend zijn. Dat is het doel, een
% beschrijving van die koppelvlakken tussen de drie machten. In identificatie en authorisatie stuk uitleggen wat
% nu eigenlijk vertrouwen is. 
% Ook uitleggen wat het omdraaien van het opslag model betekent. Kan onder storage? Macht van storage partijen,
% kans dat ze falliet gaan, dat ze gedwongen worden mee te werken met een vervelende overheid (in ons model moet 
% je akkoord gaan als de terms en conditions veranderen, anders werkt het spel domweg niet). Redundancy van de data
% beschikbaarheid van de data, de A in CIA, 
% scheiding der machten is bijna een religie, begon met de thora, wat doet de mensheid, maken er 3 godsdiensten van
% die allemaal denken toch net even wat meer gelijk te hebben dan anderen. Vandaar dat het concept van de trias politica
% zo goed werkt. Gaat erom de natuur van de mens in toom te houden. Vgl blockchain, met name bitcoin, perfect en dan toch
% 200 nieuwe coins, grondwet, elk land heeft zowat zijn eigen, je wordt het domweg niet eens en dat is iets goed maar
% wel iets dat je moet kanaliseren en dat is wat de meeste technologische oplossingen vergeten. Ethernet heeft ook zo'n
% gevecht gekend en uiteindelijk heeft het gewonnen omdat de vrijheid om zelf te beschikken erin gebakken zat en daarnaast
% het goed genoeg was. Hetzelfde geldt voor www. Dus erkennen dat er meerdere vormen, zowel centraal als decentraal,
% van identificatie zullen blijven bestaan. Erken dat er niet 1 afsprakenstelsel komt, maar heel veel. En erken dat
% niemand zijn kroonjuwelen op een plek gaat brengen dat niet 100\% juridisch, technisch en gevoelsmatig je eigen is.
% Het maakt dus niet uit (het zou niet uit moeten maken) waar de data staat, maar iedereen heeft een vrije keuze om
% zelf te bepalen waar de data komt te staan! Dus niet verplicht bij Google, maar kan ook thuis. 
% zolang die zaken niet gerelegd zijn, komt er geen generieke manier van het managen van je eigen data. Dus een succes
% vol systeem zal rekening moeten houden met deze eisen. En een uitwisselingsschema moeten zijn dat overweg kan met deze
% zaken, net zoals ethernet en ip/tcp overweg kan met een hele wereld aan verschillende zaken.
% er missen op de wereld dus nog twee zaken, een raamwerk hoe auth, id en storage samenwerken en onafhankelijke data
% opslag in de wereld. Aan dat laatste wordt hard gewerkt (DAT, BitDUST, IFS), Schluss pakt het eerste op, door het 
% combineren van zoveel mogelijk standaarden op het gebied van ID en Authorisatie.

% inleiding laat een aantal voorbeelden zien van datadeling. we hebben het namelijk over het telbaar maken van iets
% dat in de basis gekopieerd wordt en dat iedereen over dat ding (stuk data) soort zelfbeschikkings recht over wil
% hebben. We moeten laten zien met voorbeelden wat de stakeholders zijn in de datadans en waar de machten nu liggen
% daarna valt het scheiden van de machten op zijn plek.

% [ semantische web, horizon twee, vergeet twee belangrijke zaken: privacy en vrijheid (om zelf data te beheren) en 
% verdien modellen. Curated data collections geld. Waarom? data kwaliteit en data beschikbaarheid. Verder
% lost het veel van de trias politica problemen in eerste instantie op. We gaan dus pas naar een personal locker,
% wanneer we goed door de tweede horizon heen komen. Meer voor position paper!]
...
%\section{When it all goes wrong}
%Data leaks happen because the closed sets of storage, access and identity systems don't think of the scenario that a %fourth party that does not adhere to the agreement comes into play and gets hold of the data.
...
% TODO: introduce entities as Uncle Bob has defined them.
Separation of power on the Internet can be achieved if people gain independence on three separate functions:
%die drie dingen invullen en data is veilig het kunnen kiezen zorgt voor indepence
